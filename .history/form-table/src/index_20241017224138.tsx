...
const async function  handleClick =  (api: string, name: string, id: string): Promise<void> => {
  console.log(`the name and id are:${name}, ${id}`);
  const xhr = new XMLHttpRequest();
  let data: any[] = [];
  await xhr.open("GET", `https://proapi.azurewebsites.net/api/rule?token%20=%20123&current=1&pageSize=100` || `https://${api}/${name}${id}`, true);
        ...
        
          {
            type === 'tableForm' ? (
              ...
              <Button
                type="primary"
                htmlType="submit"
                onClick={handleClick(api, name, id)}
              >
                Search
              </Button>
            </Form.Item>)
